...
WITH ORDER_LOGISTICS AS (
    ...
    SELECT
        ...
        (JSON_EXTRACT_PATH_TEXT(C.SHIPPING_INFO::JSON, 'logistics_id'))::INTEGER AS LOGISTICS_ID
    FROM
        {{ ref('stg_carts') }} C
)
SELECT
    ...
    OL.COMPANY_NAME AS LOGISTICS_COMPANY,
    OL.SERVICE_TYPE AS DELIVERY_SERVICE
FROM
    ORDER_DETAILS OD
    LEFT JOIN PRODUCT_PERFORMANCE PP
    ON OD.CART_ID = PP.CART_ID
    LEFT JOIN ORDER_LOGISTICS OL
    ON OD.LOGISTICS_ID = OL.LOGISTICS_ID